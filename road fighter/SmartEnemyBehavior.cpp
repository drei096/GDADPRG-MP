...
SmartEnemyBehavior::SmartEnemyBehavior(string name, float _MULTIPLIER) : ObjectComponent(name, Script)
{
	this->SPEED_MULTIPLIER = _MULTIPLIER;
	this->reset();
}

void SmartEnemyBehavior::perform()
{
...
void SmartEnemyBehavior::configure(float delay)
{
	this->delay += delay;
}

void SmartEnemyBehavior::reset()
{
	//this->delay = (rand() % 3);
	this->movementType = Forward;
